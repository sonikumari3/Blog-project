...
const createAuthor = async function (req, res) {
    ...
    try {
        ...
        if (!authorData.password) {
            ...
            return res.status(201).send({ status: true, data: author, alert: 'Save this password to login' })
        }
...
const validateAuthor = async function (authorData) {
    ...
    if (authorData.email) {
        ...
        if (validmail) {
            errors.push(`${authorData.email} is Invalid email`)
        }
    }
    ...
    if (errors.length === 0) {
        ...
        if (userEmail) {
            errors.push(`${authorData.email} email already exist`)
        }
    }
    return errors;
};
...
const loginauthor = async function (req, res) {
    try {
        ...
        if (validmail) {
            return res.status(400).send({ status: false, msg: `${authorData.email} is Invalid email` })
        }
        ...
        if (!author) {
            return res.status(403).send({ status: false, msg: "Invalid credential" })
        }
